<template>
	<form @submit="submit" class="p-8 flex flex-col justify-center items-center relative h-screen">
		<h1 class='pb-4'>Enter URL and enter <span class='px-2 rounded border-2 border-sepia-300 dark:border-gray-700'>{{ getGlasses() }} Reader</span></h1>
		...
		<p class='text-center'>Handy bookmarklet: <a class='m-2 py-2 px-4 md:px-6 lg:px-8 rounded bg-sepia-400 dark:bg-blackop-40' href="javascript:(function()%7Bwindow.location%3D%22https%3A%2F%2Freader.adamkiss.com%2F%22%2BencodeURIComponent(window.location.href)%7D)()">{{ getGlasses() }} Reader</a></p>
	</form>
</template>
